...
def test_librarian_case3() -> None:
    query = "How many citations does the transformers paper have according to Semantic Scholar? Return only one number as a string and nothing else."
    model = LiteLLMModel(model_id="gpt-4o-mini", temperature=0.0)
    agent = get_librarian_agent(model=model)
    answer = agent(query)
    assert extract_first_number(answer.strip()) > 100000
